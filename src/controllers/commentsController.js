import { getCommentListService, submitCommentService } from "../services/commentsService.js";
import { validationResult } from "express-validator";

export const getCommentList = async (req, res) => {
    const {id} = req.params;
    try {
        const commentList = await getCommentListService(parseInt(id));
...
export const submitComment = async (req, res) => {
    ...
    try {
        ...
        if(result.success) return res.status(201).json({ message: 'Comment submited successfully!' });
